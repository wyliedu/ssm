            ...
            <div id="page-inner">
				...
				<div class="col-md-12">
					<div class="panel panel-default">
						<div class="panel-heading">承包商管理列表 
						<button class="btn btn-success pull-right addBtn"><i class="glyphicon glyphicon-plus"></i>新增承包商</button>
						</div>
						<div class="panel-body">
							<table class="table table-bordered table-striped table-hover">
								...
								<th class="text-center" style="background: #3499db; color: #ffffff; width: 10%;">承包商名称</th>
								<th class="text-center" style="background: #3499db; color: #ffffff; width: 10%;">负责人</th>
				...
				<div class="modal-body">
					<form class="form-horizontal baseform" role="form">
						...
						<div class="form-group employeediv">
							...
							<div class="col-sm-6">
							<select class="form-control col-sm-6" id="employee" name="employee">
							</select>
							</div>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<div class="col-sm-9"><input type="text" class="form-control col-sm-6" id="address" name="address"></div>
						</div>
						<div class="form-group picdiv">
							<label class="col-sm-3 control-label">负责人名称：</label>
							<div class="col-sm-6"><input type="text" class="form-control col-sm-6" id="employeename" name="employeename"></div>
						</div>
						<div class="form-group picdiv">
							<label class="col-sm-3 control-label">负责人电话：</label>
							<div class="col-sm-6"><input type="text" class="form-control col-sm-6" id="employeepayphone" name="employeepayphone"></div>
						</div>
					</form>
				</div>
...
//下拉框显示
function showContractorList(contractorid) {
	$.ajax({
...
$(function() {
	$(".table").dataTable({
		...
		ajax:  {//类似jquery的ajax参数，基本都可以用。
			...
			url: "/stb-web/contractor/contractorlist.do",
			dataType: "json",
	    ...
	    columns: [//对应上面thead里面的序列
	              ...
	              { data: function (e) {//这里给最后一列返回一个操作列表
	                    ...
	                    sContent = sContent+'<a class="btn btn-info" href="/stb-web/contractor/employee?contractorid='+e.contractorid+'">员工</a>&nbsp;&nbsp;&nbsp;<button class="btn btn-danger delete">删除</button>'; 
	                    return sContent;
	                }, "sClass" : "text-center"
	              }
	    ] ,
...
$(document).on('click','.using',function(){
	...
	$.ajax( {
		type:"post",//不写此参数默认为get方式提交
		async:false,   //设置为同步
		url : "/stb-web/contractor/usingContractor.do",//请求的uri
		data : {contractorid:aData.contractorid},//传递到后台的参数				
		cache : false,
...
$(document).on('click','.delete',function(){
	if (confirm("你确定要删除该承包商吗?"))  { 
	aData = $(".table").DataTable().row($(this).parents("tr")).data();
	$.ajax( {
		type:"post",//不写此参数默认为get方式提交
		async:false,   //设置为同步
		url : "/stb-web/contractor/deleteContractor.do",//请求的uri
		data : {contractorid:aData.contractorid},//传递到后台的参数				
		cache : false,
		dataType : 'text',//后台返回前台的数据格式为json
		success : function(data) {
...
$(document).on('click','.modify',function(){//修改方案
	...
	showContractorList(aData.contractorid)
	$('#name').val(aData.name);
	$('#employee').val(aData.picid);
	$('#employee').selectpicker('refresh');
	$('#payphone').val(aData.payphone);
	$('#address').val(aData.address);
	$('.modal-title').html("修改承包商");
...
$(".summit").click(function () {
	var id = 0;
	if(operationType != 1){
		id = aData.contractorid;
	}
	if($(".baseform").valid()){
		$.ajax( {
			type:"post",//不写此参数默认为get方式提交
			async:false,   //设置为同步
			url : "/stb-web/contractor/updateContractor.do",//请求的uri
			data : {
				type:operationType,
				contractorid:id,
				picid:$('#employee').val(),
				...
				address:$('#address').val(),
				picname:$('#employee').find("option:selected").text(),
				employeename:$('#employeename').val(),
				employeepayphone:$('#employeepayphone').val()
				},//传递到后台的参数				
			cache : false,
			dataType : 'json',//后台返回前台的数据格式为json
			success : function(data) {
